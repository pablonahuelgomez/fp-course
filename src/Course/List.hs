...
flatten ::
  List (List a)
  -> List a
flatten = foldRight (\xs xss -> xs ++ xss) Nil
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f = foldRight (\x rs -> f x ++ rs) Nil
...
seqOptional (h :. t) =
  case h of
    Empty    -> Empty
    (Full x) -> consOptional x (seqOptional t)

consOptional :: a -> Optional (List a) -> Optional (List a)
consOptional x (Full xs) = Full (x :. xs)
